...
//        Find all books with publisher 'O'Reilly Media' containing title 'Javascript'
        ElementsCollection bookList = $("div.rt-tbody")
                .findAll(By.xpath("//span[contains (@id,\"JavaScript\")]/../../../div[4][text()=\"O'Reilly Media\"]/../.."));
        for (int i = 0; i < bookList.size(); i++) {
            System.out.println("Book " + (i + 1) + " >> " + bookList.get(i).text());
            System.out.println("----");
        }
        System.out.println();

//        Check that size of returned list equals to 10
        softAssert.assertEquals(bookList.size(), 10, "array size is not 10");

//      Check that first match row's title equals to 'Learning JavaScript Design Patterns'
        for (int i = 0; i < bookList.size(); i++) {
            if (i == 0) {
                softAssert.assertEquals(bookList.get(i).find("span").getText(), "Learning JavaScript Design Patterns");
                System.out.println("First row title is: " + bookList.get(i).find("span").getText());
            }
        }
        System.out.println("-----------------------------------");

//        Using stream() click on all matching row's title
        bookList.stream().forEach(y -> {
            y.find(".mr-2 > a").click();
